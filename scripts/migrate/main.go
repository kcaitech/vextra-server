...
func main() {
	...
	if err := sourceDB.Table("document").Where("id > ? AND deleted_at is null AND version_id is not null", 150354936838766592).Find(&oldDocuments).Error; err != nil {
		log.Fatalf("Error querying documents: %v", err)
	}
	// var documentIds []int64 189660612962304000 - 287543299479191552 较多的 对象上传错误1
