...
call add_ssh_key.bat root@192.168.0.123 22
...
for /L %%i in (0,1,%end%) do (
    set port=!ports[%%i]!
    powershell.exe -NoProfile -Command "$process = Start-Process -FilePath 'ssh' -ArgumentList '-N -L !port! root@192.168.0.123' -NoNewWindow -PassThru; $process.Id | Out-File ssh_pids.txt"
)
